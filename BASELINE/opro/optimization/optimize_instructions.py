...
def main(_):
  ...
  if scorer_llm_name in {"gpt-3.5-turbo", "gpt-5-nano"}:
  ...
  if optimizer_llm_name in {"gpt-3.5-turbo", "gpt-5-nano"}:
    assert openai_api_key, "The OpenAI API key must be provided."
    openai.api_key = openai_api_key
  else:
  ...
  if scorer_llm_name == "text-bison":
    ...
    assert scorer_llm_name.lower() in {"gpt-3.5-turbo", "gpt-5-nano"}
  ...
  if optimizer_llm_name.lower() == "text-bison":
    ...
    assert optimizer_llm_name in {"gpt-3.5-turbo", "gpt-5-nano"}
  ...
  if scorer_llm_name == "text-bison":
    ...
    assert scorer_llm_name in {"gpt-3.5-turbo", "gpt-5-nano"}
