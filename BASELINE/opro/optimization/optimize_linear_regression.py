...
_OPTIMIZER = flags.DEFINE_string(
    "optimizer", "gpt-3.5-turbo", "The LLM model to use for optimization"
)
...
def main(_):
  ...
  assert optimizer_llm_name in valid_openai_models.union(valid_palm_models), (
      f"Model '{optimizer_llm_name}' is not supported. "
      f"Valid OpenAI models: {valid_openai_models}. "
      f"Valid PaLM models: {valid_palm_models}."
  )
  openai_api_key = _OPENAI_API_KEY.value
  palm_api_key = _PALM_API_KEY.value

  if optimizer_llm_name in valid_openai_models:
    assert openai_api_key, "The OpenAI API key must be provided."
    openai.api_key = openai_api_key
  else:
    assert optimizer_llm_name in valid_palm_models
    assert (
        palm_api_key
  ...
  if optimizer_llm_name.lower() == "text-bison":
    ...
    assert optimizer_llm_name in valid_openai_models
    optimizer_gpt_max_decode_steps = 1024
    optimizer_gpt_temperature = 1.0

    optimizer_llm_dict = dict()
    optimizer_llm_dict["model_type"] = optimizer_llm_name
  ...
  for i_rep in range(num_reps):
    ...
    for i_step in range(max_num_steps):
      ...
      while remaining_num_points_to_generate > 0:
        try:
          new_outputs = call_optimizer_server_func(meta_prompt)
          if new_outputs:
            raw_outputs += new_outputs
            print(f"raw outputs: {raw_outputs}")
          else:
            print("Warning: Received empty output from optimizer")
