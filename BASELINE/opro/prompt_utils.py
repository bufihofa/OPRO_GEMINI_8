...
try:
  ...
  OPENAI_V1 = True  # New API (>= 1.0.0)
except ImportError:
  OPENAI_V1 = False  # Old API (< 1.0.0)

# Initialize OpenAI client (for new API only)
openai_client = None

def init_openai_client():
  """Initialize OpenAI client for the new API."""
  global openai_client
  if openai_client is None and OPENAI_V1:
...
def call_openai_server_single_prompt(
  ...
  try:
    ...
    if "timeout" in str(e).lower() or (hasattr(e, "__class__") and "timeout" in e.__class__.__name__.lower()):
      retry_time = 30
    ...
    elif "rate" in str(e).lower() or (hasattr(e, "__class__") and "rate" in e.__class__.__name__.lower()):
      retry_time = 30
    ...
    # Handle API errors
    elif "api" in str(e).lower() or (hasattr(e, "__class__") and "api" in e.__class__.__name__.lower()):
      retry_time = 30
